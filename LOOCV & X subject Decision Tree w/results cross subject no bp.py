...
# File path
file_path = r"D:\aria-seated\Results\cross_subject_testing_results_decision_tree_no_bp.csv"
...
summary_df.to_csv("cross_subject_metrics_summary.csv")

# Plot Mean and Median as a Bar Chart
plt.figure(figsize=(10, 5))
summary_df.plot(kind='bar', figsize=(10, 6))
plt.title("Mean and Median of Evaluation Metrics (Cross-Subject Decision Tree No BP)")
plt.xlabel("Metrics")
plt.ylabel("Values")
...
plt.savefig("cross_subject_metrics_summary.png", dpi=300)
plt.show()
...
for i in range(num_chunks):
    ...
    plt.title(f"Line Chart of Instances {start_idx} to {end_idx - 1} (Cross-Subject Decision Tree No BP)")
    ...
    plt.savefig(f"cross_subject_metrics_line_chart_{i + 1}.png", dpi=300)
    plt.show()
